...
int lesson_10_halide(struct halide_buffer_t *_p0_buffer, uint8_t _offset, struct halide_buffer_t *_brighter_buffer);

HALIDE_FUNCTION_ATTRS
int lesson_10_halide_argv(void **args);

HALIDE_FUNCTION_ATTRS
const struct halide_filter_metadata_t *lesson_10_halide_metadata();
...
const char* const lesson_10_halide_kwlist[] = {
  "p0",
  "offset",
  "brighter",
  nullptr
...
// lesson_10_halide
PyObject *lesson_10_halide(PyObject *module, PyObject *args, PyObject *kwargs) {
  PyObject* py_p0;
  unsigned int py_offset;
  PyObject* py_brighter;
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OIO", (char**)lesson_10_halide_kwlist
    , &py_p0
    , &py_offset
    , &py_brighter
  )) {
  ...
  PyHalideBuffer<2> b_p0;
  PyHalideBuffer<2> b_brighter;
  if (!b_p0.unpack(py_p0, 0, lesson_10_halide_kwlist[0])) return nullptr;
  if (!b_brighter.unpack(py_brighter, PyBUF_WRITABLE, lesson_10_halide_kwlist[2])) return nullptr;

  b_p0.halide_buf.set_host_dirty();
  int result;
  Py_BEGIN_ALLOW_THREADS
  result = lesson_10_halide(
    &b_p0.halide_buf,
    py_offset,
    &b_brighter.halide_buf
  );
  Py_END_ALLOW_THREADS
  if (result == 0) result = halide_copy_to_host(nullptr, &b_brighter.halide_buf);
  if (result != 0) {
...
#define HALIDE_PYTHON_EXTENSION_MODULE_NAME lesson_10_halide
#endif  // HALIDE_PYTHON_EXTENSION_MODULE_NAME

#ifndef HALIDE_PYTHON_EXTENSION_FUNCTIONS
#define HALIDE_PYTHON_EXTENSION_FUNCTIONS X(lesson_10_halide)
