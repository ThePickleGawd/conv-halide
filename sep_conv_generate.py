...
def main():
    ...
    kernel = hl.ImageParam(hl.Float(32), 3, "kernel")  # (KW, KH, C_in)
    ...
    pw[x, y, c, n] = bias[c]
    pw[x, y, c, n] += dw[x, y, rc, n] * pw_filter[c, rc]
    ...
    if test:
        ...
        kernel_np = np.zeros((C_in, KH, KW), dtype=np.float32)  # (C_in, KH, KW)
        pw_filter_np = np.zeros((C_in, C_out), dtype=np.float32)  # (C_in, C_out)
        bias_np = np.zeros((C_out), dtype=np.float32)  # (C_out)
        ...
        out = sep_conv.realize([8, 8, 3, 2])  # (W, H, C_out, N)
